...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addInkAnnotationInternal)(JNIEnv * env, jobject thiz, jobjectArray arcs, jfloatArray color, jfloat thickness)
{//modified
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	...
	color1 = (*env)->GetFloatArrayElements(env, color, 0);//modified
	...
	fz_try(ctx)
	{
		...
		pdf_set_ink_annot_list(idoc, (pdf_annot *)annot, pts, counts, n, color1, thickness);
